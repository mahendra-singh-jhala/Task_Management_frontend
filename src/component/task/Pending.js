...
const Pending = () => {
    ...
    if (task && Array.isArray(task)) {
        pending = task?.filter(task => task.status === "active")
    }
    ...
    return (
        <div className="my-8 mx-10">
            <h1 className="text-2xl text-white font-bold uppercase border-b-2 border-white p-2">Pending Tasks</h1>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
                {pending && pending.length >= 0 && pending.map((task) => (
